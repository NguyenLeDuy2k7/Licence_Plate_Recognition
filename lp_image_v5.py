...
class Config:    
    YOLO_PATH = 'yolov5'
    LP_DETECTOR_MODEL = 'model/LP_detector_v5.pt'
    LP_OCR_MODEL = 'model/LP_ocr_v5.pt'
...
def detect_license_plates(detector, image):
    results = detector(image, size=Config.DETECTION_SIZE)
...
def process_image_directly(image, ocr_model):
    plate_text = helper_v5.read_plate(ocr_model, image)
